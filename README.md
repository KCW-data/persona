...
5. 페르소나가 실제 사회조사 응답자처럼 각 현안에 대해 1~5점 척도로 응답한다.
...
현안 응답 생성에서는 입력 문항의 사회 현안 태도를 앵커로 사용합니다. 10개 현안이 모두 3점으로 나오는 응답은 실패로 처리되며, 3점 응답은 최대 2개까지만 허용합니다.

## Gemini API와 관리자 대시보드

일반 사용자는 Gemini API 키를 입력하지 않습니다. 강의자는 Streamlit Cloud Secrets에 키를 한 번만 설정합니다.
...
이메일은 해시 처리하지 않고 입력한 주소 그대로 저장합니다. 관리자 대시보드에서 이메일별 기록 삭제 또는 전체 테스트 데이터 삭제를 할 수 있습니다.
